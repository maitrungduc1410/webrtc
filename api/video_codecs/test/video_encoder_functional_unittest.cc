...
class VideoEncoderFunctionalTest
 ...
 protected:
  ...
  void SetUp() override {
    factory_ = GetParam()();
    decoder_factory_ = CreateBuiltinVideoDecoderFactory();
  }
...
TEST_P(VideoEncoderFunctionalTest, EncodesAndDecodesKeyframe) {
  ...
  ASSERT_TRUE(dec.IsSupported())
      << "No matching decoder found for codec: " << factory_->CodecName();
...
TEST_P(VideoEncoderFunctionalTest, SupportsAllReferenceBuffers) {
  ...
  ASSERT_TRUE(combined_dec.IsSupported())
      << "No matching decoder found for codec: " << factory_->CodecName();
