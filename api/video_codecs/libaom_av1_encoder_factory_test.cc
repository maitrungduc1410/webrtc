...
TEST(LibaomAv1EncoderFactory, ReportsCorrectCapabilities) {
  ...
  auto capabilities = factory.GetEncoderCapabilities();

  // Prediction Constraints
  const auto& pc = capabilities.prediction_constraints();
  EXPECT_EQ(pc.num_buffers(), 8);
  EXPECT_EQ(pc.max_references(), 3);
  EXPECT_EQ(pc.max_temporal_layers(), 4);
  EXPECT_EQ(pc.buffer_space_type(),
  ...
  // Scaling Factors
  const auto& scaling_factors = pc.scaling_factors();
  ...
  const auto& frame_types = pc.supported_frame_types();
  ...
  const auto& input_formats = ic.input_formats();
  ...
  EXPECT_EQ(rc_modes[0], VideoEncoderFactoryInterface::RateControlMode::kCbr);
  EXPECT_EQ(rc_modes[1], VideoEncoderFactoryInterface::RateControlMode::kCqp);

  // Performance
  const auto& perf = capabilities.performance();
