...
TEST_F(VideoStreamEncoderTest,
       ConfigureEncoderRequestsRefreshFrameOnScaleResolutionDownByChange) {
  MockVideoSourceInterface mock_source;
  VideoSinkInterface<VideoFrame>* input_sink = nullptr;
  EXPECT_CALL(mock_source, AddOrUpdateSink)
      .WillRepeatedly(::testing::SaveArg<0>(&input_sink));
  EXPECT_CALL(mock_source, RemoveSink).WillRepeatedly(Return());
  ...
  VideoEncoderConfig config;
  test::FillEncoderConfiguration(kVideoCodecVP8, 1, &config);
  config.simulcast_layers[0].scale_resolution_down_by = 1.0;
  config.video_stream_factory = nullptr;
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);

  // Send a frame to establish resolution and trigger initial configuration.
  ASSERT_TRUE(input_sink);
  input_sink->OnFrame(CreateFrame(1, 1280, 720));
  WaitForEncodedFrame(1);

  // Expect RequestRefreshFrame when we change scale_resolution_down_by.
  EXPECT_CALL(mock_source, RequestRefreshFrame).Times(1);

  // Change scale_resolution_down_by.
  config.simulcast_layers[0].scale_resolution_down_by = 2.0;
  config.video_stream_factory = nullptr;  // Ensure factory is still null
  video_stream_encoder_->ConfigureEncoder(std::move(config), kMaxPayloadLength);

  // Deplete task queues to ensure the task runs.
  video_stream_encoder_->WaitUntilTaskQueueIsIdle();
  AdvanceTime(TimeDelta::Millis(0));
  video_stream_encoder_->Stop();
}
...
TEST_F(VideoStreamEncoderTest,
       ConfigureEncoderDefersReconfigurationOnScaleResolutionDownToChange) {
  video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(
      kTargetBitrate, kTargetBitrate, 0, 0, 0);
  ...
  VideoEncoderConfig config;
  test::FillEncoderConfiguration(kVideoCodecVP8, 1, &config);
  config.simulcast_layers[0].scale_resolution_down_to = {.width = 640,
                                                         .height = 360};
  video_stream_encoder_->ConfigureEncoder(config.Copy(), kMaxPayloadLength);

  // Send a frame to ensure encoder is initialized and active.
  int width = 1280;
  int height = 720;
  video_source_.IncomingCapturedFrame(CreateFrame(1, width, height));
  WaitForEncodedFrame(1);

  // Reconfiguration deferred until next frame.
  int initial_init_count = fake_encoder_.GetNumInitializations();

  // Change scale_resolution_down_to.
  config.simulcast_layers[0].scale_resolution_down_to = {.width = 320,
                                                         .height = 180};
  video_stream_encoder_->ConfigureEncoder(std::move(config), kMaxPayloadLength);
  AdvanceTime(TimeDelta::Millis(1));

  EXPECT_EQ(fake_encoder_.GetNumInitializations(), initial_init_count);

  // Frame triggering reconfiguration.
  video_source_.IncomingCapturedFrame(CreateFrame(2, width, height));
  WaitForEncodedFrame(2);

  EXPECT_EQ(fake_encoder_.GetNumInitializations(), initial_init_count + 1);
  video_stream_encoder_->Stop();
}
